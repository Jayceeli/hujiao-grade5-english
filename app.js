let BOOK=null,MANIFEST=null,currentUnit=0,stopAt=null,playStart=null,activeLine=null;
const $=s=>document.querySelector(s); const audio=$('#audio');
const htmlEsc=s=>String(s??'').replace(/[&<>"']/g,c=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#039;'}[c]));
async function init(){
  [BOOK,MANIFEST]=await Promise.all([fetch('data/book.json').then(r=>r.json()),fetch('data/audio-sprite-manifest.json').then(r=>r.json())]);
...
function renderNav(){const nav=$('#nav');nav.innerHTML='';BOOK.units.forEach((u,i)=>{const b=document.createElement('button');b.textContent=u.title;b.dataset.i=i;b.onclick=()=>renderUnit(i);nav.appendChild(b)});}
...
$('#search').addEventListener('input',e=>{const q=e.target.value.trim().toLowerCase();if(!q){renderUnit(currentUnit);return}const root=$('#content');root.innerHTML='';BOOK.units.forEach(u=>u.sections.forEach(s=>{if((s.title+' '+(s.text||'')).toLowerCase().includes(q)){const c=sectionCard(s);const label=document.createElement('div');label.className='search-unit';label.textContent=u.title;c.querySelector('.section-body').prepend(label);root.appendChild(c)}}));$('#title').textContent='搜索结果';$('#range').textContent=`关键词：${q}`;});
